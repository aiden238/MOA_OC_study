...
- 전체 테스트는 현재 `132 passed` 상태임

### 현재 한계
- RAG는 `ChromaRetriever` + `OpenAIEmbedder` 경로가 코드에 반영되었지만, 실환경 실행 산출물 증거가 아직 없음
- `Embedder`는 로컬 폴백용 해시 기반 구현이고, 런타임 기본 경로는 `OpenAIEmbedder`로 전환됨
- MCP는 `mock://local` 수준이며, 실제 MCP 프로토콜 세션/서버 연동이 아님
- `run_full.py` 경로에서 `routing` 정보 전달은 수정되었지만, `evaluation`이 아직 실행되지 않아 비교 실험 출력이 불완전함
...
> Week 7 이후에는 커밋 횟수를 제한하지 않되, 기능 단위와 회귀 추적 단위는 유지한다.
